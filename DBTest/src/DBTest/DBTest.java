...
//		            }
					...
					// phoneInfo 테이블 생성
					query = "CREATE TABLE IF NOT EXISTS recDB.phoneInfo("
							+" phoneName varchar(45) NOT NULL,"
							+" company varchar(45) NOT NULL,"
							+" phonePrice int(11) NOT NULL,"
							+" screenSize decimal(5,3) NOT NULL,"
							+" os varchar(45) NOT NULL,"
							+" capacity int(11) NOT NULL,"
							+" RAM int(11) NOT NULL,"
							+" frontCamera int(11) NOT NULL,"
							+" rearCamera int(11) NOT NULL,"
							+" weight int(11) NOT NULL,"
							+" phoneSize varchar(45) NOT NULL,"
							+" betteryCapacity int(11) NOT NULL,"
							+" speed float NOT NULL,"
							+" resolution varchar(45) NOT NULL,"
							+" releaseYear int(11) NOT NULL,"
							+" color varchar(70) NOT NULL,"
							+" link varchar(150) NOT NULL,"
							+" recCount int(11) NOT NULL DEFAULT 0,"
		            ...
		            // planInfo 테이블 생성
					query = "CREATE TABLE IF NOT EXISTS recDB.planInfo ("
							...
							+" phoneOption varchar(70) NOT NULL,"
			...
			public void setPhoneDB(String[] input) {
				try{
					...
					// 쿼리문 작성 
//					query = "INSERT INTO phoneInfo(phoneName, company, phonePrice, screenSize, os, "
//							+ "capacity, RAM, frontCamera, rearCamera, weight, phoneSize, betteryCapacity, speed, resolution, releaseYear, color, link,link2)" 
//							+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
//		
...
//					ppstatement.setString(18, input[17]); // 데이터 형식 수정 필요 ***
					...
					query = "INSERT INTO phoneInfo(phoneName, company, phonePrice, screenSize, os, "
							+ "capacity, RAM, frontCamera, rearCamera, weight, phoneSize, betteryCapacity, speed, resolution, releaseYear, color, link,link2)" 
							+ " VALUES('" + input[0] + "', '" + input[1] + "', '" + Integer.parseInt(input[2]) + "', '" + Double.parseDouble(input[3]) 
							+ "', '" + input[4] + "', '" + Integer.parseInt(input[5]) + "', '" + Integer.parseInt(input[6]) + "', '" + Integer.parseInt(input[7]) 
							+ "', '" + Integer.parseInt(input[8]) + "', '" + Integer.parseInt(input[9]) + input[10] + "', '" + Integer.parseInt(input[11]) 
							+ "', '" + Double.parseDouble(input[12]) + input[13] + "', '" + Integer.parseInt(input[14]) + input[15] + "', '" + input[16] 
							+ "', '" + Integer.parseInt(input[17]) + "', '" + input[18] + "')";
			...
			public void setPlanDB(String[] input) {
				try{
					...
					Statement statement = connection.createStatement();
					// 쿼리문 작성
					query = "INSERT INTO planinfo(operator, planName, mobile, planPrice, data, videoCall, phoneCall, message, phoneOption, data2)"
							+ " VALUES('" + input[0] + "', '" + input[1] + "', '" + input[2] + "', '" + Integer.parseInt(input[3]) 
			...
			public void setCustomerDB(String[] input) {
				try{
					...
					Statement statement = connection.createStatement();
					// 쿼리문 작성
					query = "INSERT INTO customerInfo(ID, password, name, age, phone_number, email_address, sex)"
